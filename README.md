...
This section mirrors the Postman collection in bkash.json. Replace the placeholders with your own credentials.

### Base URL

```
https://tokenized.{sandbox|pay}.bka.sh/v2/tokenized-checkout
```

Postman pre-request logic builds `{{bkash_base_url}}` from `{{bkash_env}}`:

```
bkash_env=sandbox | pay | production | live
bkash_base_url=https://tokenized.{sandbox|pay}.bka.sh/v2/tokenized-checkout
```

### Shared Headers

Non-auth endpoints include these headers:
...
Auth endpoints use these headers instead of `authorization` and `x-app-key`:
...
### Postman Variables (from bkash.json)

| Variable | Description | Example |
| --- | --- | --- |
| bkash_env | Environment selector | sandbox |
| bkash_env_resolved | Resolved env used by Postman | sandbox |
| bkash_base_url | Base API URL | https://tokenized.sandbox.bka.sh/v2/tokenized-checkout |
| bkash_app_key | App key | YOUR_APP_KEY |
| bkash_app_secret | App secret | YOUR_APP_SECRET |
| bkash_username | API username | YOUR_USERNAME |
| bkash_password | API password | YOUR_PASSWORD |
| bkash_id_token | Access token from grant/refresh | YOUR_ID_TOKEN |
| bkash_refresh_token | Refresh token from grant/refresh | YOUR_REFRESH_TOKEN |
| amount | Payment amount | 100 |
| currency | Currency code | BDT |
| intent | Payment intent | sale |
| merchant_invoice_number | Invoice number (unique) | inv_1715155200000 |
| payer_reference | Customer reference | payer_1715155200000 |
| merchant_association_info | Merchant info | woocommerce |
| payment_id | Payment ID | payment_123 |
| agreement_id | Agreement ID | agreement_123 |
| trx_id | Transaction ID | trx_123 |
| payment_redirect_url | Redirect URL from create | https://tokenized.sandbox.bka.sh/checkout/... |
| refund_amount | Refund amount | 10 |
| sku | SKU or invoice reference | order-sku |
| reason | Refund reason | Manual API test |
| callback_url | Payment callback URL | https://merchant.example.com/bkash/payment-callback |
| agreement_callback_url | Agreement callback URL | https://merchant.example.com/bkash/agreement-callback |
| webhook_receiver_url | Webhook receiver URL | https://merchant.example.com/bkash/webhook |
| sns_topic_arn | SNS topic ARN | arn:aws:sns:us-east-1:123456789012:bkash-topic |
| sns_subscribe_url | SNS subscription URL | https://sns.us-east-1.amazonaws.com/?Action=ConfirmSubscription&TopicArn=... |
| sns_token | SNS token | EXAMPLE_TOKEN |
| sns_signature | SNS signature | EXAMPLE_SIGNATURE |
| sns_signing_cert_url | SNS cert URL | https://sns.us-east-1.amazonaws.com/SimpleNotificationService.pem |
| debit_msisdn | Sender MSISDN | 017XXXXXXXX |
| credit_shortcode | Merchant shortcode | 12345 |
| transaction_type | Notification transaction type | Payment |
| transaction_status | Notification status | Completed |
| bkash_sdk_version | SDK version | 1.2.0-beta |
| bkash_sdk_host | SDK host | sandbox |
| bkash_sdk_suffix | SDK suffix | -sandbox |
